...
44. **Un `Text`/`Span` sin tamaño mide 14px**, mientras que en web heredaba los 16px/24 del body. Pasó con el texto "editar fondo" y con la variante `secundario` del `Boton` (que no traía tamaño): hay que poner `text-base leading-normal` donde el original heredaba del body.
...
`(public)` está terminado. Sigue `(protected)`: carrito, pedidos, perfil y todo el lado del distribuidor. Es más grande y su layout todavía usa el `Header`/`BottomNav` viejos de web. Confirmar con el usuario por dónde empezar.
