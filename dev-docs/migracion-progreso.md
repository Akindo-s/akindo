...
45. **Un `Text`/`Span` sin tamaño mide 14px**, mientras que en web heredaba los 16px/24 del body. Pasó con el texto "editar fondo" y con la variante `secundario` del `Boton` (que no traía tamaño): hay que poner `text-base leading-normal` donde el original heredaba del body.
...
Confirmar con el usuario. De `(protected)` quedan `carrito/preorden`, `pedidos` (y `pedidos/ordenes`, `pedidos/[pedidoId]`), `perfil`, `admin/categorias` y todo el lado del distribuidor. Las dos siguientes naturales son `perfil` y `pedidos`, porque son tabs del BottomNav: al migrarlas hay que agregar su href a `HREFS_MIGRADOS` en `app/(protected)/_layout.tsx`.
