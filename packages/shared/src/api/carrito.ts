...
export async function fetchCarritosRaw(token?: string): Promise<RawCarrito[]> {
...
export async function obtenerIdsCarrito(token?: string): Promise<string[]> {
  const carrito: RawCarrito[] = await fetchCarritosRaw(token);
...
export async function obtenerCarritoCliente(token?: string): Promise<CarritoUiData> {
  const carritos = await fetchCarritosRaw(token);
  return toUiData(carritos);
}

export async function verificarProductoEnCarrito(
  productoId: string,
  token?: string
): Promise<boolean> {
  try {
...
export async function actualizarCantidadCarrito(
  ...
  try {
    ...
    const data = await obtenerCarritoCliente(token);
...
export async function vaciarCarritosCliente(token?: string): Promise<CarritoActionResult> {
  try {
    const carritos = await fetchCarritosRaw(token);
